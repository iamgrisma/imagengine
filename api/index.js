...
function getOptions(query, cleanPath) {
  ...
  const isOg =
    cleanPath.includes('/og/') ||
    cleanPath.endsWith('/og') ||
    /(?:^|\/)og(?:-|\.|$)/i.test(cleanPath);
  ...
  return {
    avatar,
    isOg,
    width: width ? parseInt(width, 10) : avatar ? 256 : isOg ? 1200 : null,
    height: height ? parseInt(height, 10) : avatar ? 256 : isOg ? 630 : null,
    quality: rawQuality
